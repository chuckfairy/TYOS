...
class Run():
    def __init__(self, fona):
        ...
        self.stock_image = pygame.image.load(APP_PATH + self.app_order[0] + '/' + self.app_order[0] + '.png')
    ...
    def load_icons(self):
        for i in range(0, len(self.app_order)):
            #Add icon and rect
            self.blit['surfaces'].append(pygame.image.load(APP_PATH + self.app_order[i] + '/' + self.app_order[i] + '.png'))
    ...
    def get_app_order(self):
        ...
        order_file = open(APP_PATH + 'order.txt', 'r')
